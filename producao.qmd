...
```nginx
# /etc/nginx/sites-available/verificadocscredito
...
```bash
sudo ln -s /etc/nginx/sites-available/verificadocscredito /etc/nginx/sites-enabled/
...
if [ "$HEALTH" != "200" ]; then
  echo "$(date) ALERTA: INJI Verify Service indisponível (HTTP $HEALTH)" \
    >> /var/log/verificadocscredito-monitor.log
