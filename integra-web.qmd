...
Implementar uso de credenciais verificáveis em uma aplicação Web React usando QR Code (fluxo cross-device).
...
sequenceDiagram
    ...
    U->>SDK: Clica "Verificar Documentos de Crédito Rural"
...
type VerificaDocsCreditoProps = {
  onVerificado: () => void;
  onFalhou: () => void;
};

export function VerificaDocsCredito({ onVerificado, onFalhou }: VerificaDocsCreditoProps) {
...
export function Checkout() {
  return (
    <div>
      <h1>Verificação de Documentos de Crédito Rural</h1>
      <VerificaIdade
...
```tsx
// src/components/VerificaDocsCreditoProd.tsx
import { OpenID4VPVerification } from '@mosip/react-inji-verify-sdk';

export function VerificaDocsCreditoProd() {
...
<OpenID4VPVerification
  ...
  triggerElement={<button className="btn-verificar">Verificar Documentos de Crédito Rural</button>}
/>
```
